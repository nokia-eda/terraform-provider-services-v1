...
import (
	...
	"github.com/nokia/eda/apps/terraform-provider-services/internal/resource_virtual_network"
	"github.com/nokia/eda/apps/terraform-provider-services/internal/tfutils"
)

const (
	create_rs_virtualNetwork = "/apps/services.eda.nokia.com/v1/namespaces/{namespace}/virtualnetworks"
	read_rs_virtualNetwork   = "/apps/services.eda.nokia.com/v1/namespaces/{namespace}/virtualnetworks/{name}"
	update_rs_virtualNetwork = "/apps/services.eda.nokia.com/v1/namespaces/{namespace}/virtualnetworks/{name}"
	delete_rs_virtualNetwork = "/apps/services.eda.nokia.com/v1/namespaces/{namespace}/virtualnetworks/{name}"
)
...
func (r *virtualNetworkResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = resource_virtual_network.VirtualNetworkResourceSchema(ctx)
}

func (r *virtualNetworkResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	var data resource_virtual_network.VirtualNetworkModel
	...
	err = r.client.Create(ctx, create_rs_virtualNetwork, map[string]string{
	...
	err = r.client.Get(ctx, read_rs_virtualNetwork, map[string]string{
		"namespace": tfutils.StringValue(data.Metadata.Namespace),
		"name":      tfutils.StringValue(data.Metadata.Name),
	}, &result)

	tflog.Info(ctx, "Read()::API returned", map[string]any{
		"path":      read_rs_virtualNetwork,
		"result":    spew.Sdump(result),
		"timeTaken": time.Since(t0).String(),
	})
...
func (r *virtualNetworkResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	err = r.client.Update(ctx, update_rs_virtualNetwork, map[string]string{
		"namespace": tfutils.StringValue(data.Metadata.Namespace),
		"name":      tfutils.StringValue(data.Metadata.Name),
	}, reqBody, &result)
...
func (r *virtualNetworkResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err := r.client.Delete(ctx, delete_rs_virtualNetwork, map[string]string{
		"namespace": tfutils.StringValue(data.Metadata.Namespace),
		"name":      tfutils.StringValue(data.Metadata.Name),
	}, &result)
